...
Defender::Defender(SDL_Renderer* renderer, int teamColor, SDL_Point* position, int team, int difficulty, double angle, bool selected) :
...
void Defender::update(SDL_Event* e, const Uint8* keyboardState, Object** obj, bool* flag)
{
	...
	else
	{
		...
		{
			//Ball is in FREE / SHOOT state. If the defender is the closest in his team, he will go towards the ball
			...
			{
				//Defender will go on a different Y coord as the playerToFollow, and X coord dependingly on the half the ball is in 
				if (dynamic_cast<Ball*>(obj[BALL])->getCurrentPlayer() != this)
				{
					if(abs(Player::getPosition()->x - initialPosition->x) > 5 || abs(Player::getPosition()->x - initialPosition->x) > 5)
					{
						updateCoord(initialPosition, speed, dx, dy);
						Player::getTexture()->setAngle(getTextureAngle(Player::getPosition(), initialPosition));
						Player::setMovingState(true);
					}
					else
					{
